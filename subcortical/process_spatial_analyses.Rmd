...
sa_reg <- loadRegistry("spatial_analyses")
```
...
process_randomization <- function(mod){
        ...
        steps <- seq(0,max_t, length.out = 90)
...
if(!nrow(findDone()) == 18018){
  stop("Something is amiss, there should be 18018 finished jobs, "
     , "and 18 fails (shuffle 1001 doesn't exist)")
}
sa_res <-
  randomization_experiments %>%
  ungroup %>% ## Extract all complete 
  mutate(job_id = findJobs(reg = sa_reg)$job.id) %>%
  filter(job_id %in% findDone(reg = sa_reg)$job.id) %>%
